...
class TrainingSet:
    ...
    def get_train_data(self, data_properties):
        """
        Returns only the training data (65%) ready to be used as input of a classification/regression model.
        ...
        Return:
        1. list of all unique labels
        2. Numpy array of training entries (65%)
        3. Numpy array of training entries' labels
        """
        all_data_info, labels_set = self._load(data_to_load='train')
        self.labels_set = labels_set
        ...
        X_train = data.to_numpy().astype('float32')
        y_train = labels.to_numpy().astype('str')
        ...
        return self.labels_set, X_train, y_train

    def get_test_data(self, data_properties):
        """
        Returns only the test data (35%) ready to be used for evaluation.
        ...
        Return:
        1. list of all unique labels
        2. Numpy array of test entries (35%)
        3. Numpy array of test entries' labels
        4. Pandas DataFrame with full test data info (for detailed analysis)
        """
        all_data_info, labels_set = self._get_test_set()
        ...
        if self.labels_set is not None:
            ...
            all_data_info = all_data_info.loc[all_data_info['Label'].isin(labels_set)]

        data = all_data_info.iloc[:, ~all_data_info.columns.isin(['Data Set Name', 'Cluster ID', 'Label'])]
        labels = all_data_info['Label']

        X_test = data.to_numpy().astype('float32')
        y_test = labels.to_numpy().astype('str')

        X_test = np.nan_to_num(X_test)

        return labels_set, X_test, y_test, all_data_info

    def get_data_split_info(self):
        """
        Returns information about the train/test split.
        
        Return:
        dict with split information
        """
        all_data_info_train, _ = self._load(data_to_load='train')
        all_data_info_test, _ = self._get_test_set()
        
        total = all_data_info_train.shape[0] + all_data_info_test.shape[0]
        
        return {
            'train_size': all_data_info_train.shape[0],
            'test_size': all_data_info_test.shape[0],
            'total_size': total,
            'train_percentage': round(all_data_info_train.shape[0] / total * 100, 1) if total > 0 else 0,
            'test_percentage': round(all_data_info_test.shape[0] / total * 100, 1) if total > 0 else 0,
            'split_level': self.test_set_level,
            'test_ids_count': len(self.test_set_ids)
        }
    ...
    def _load(self, data_to_load):
        ...
        nb_dropped_cols = nb_cols - all_complete_datasets_df.shape[1]
