...
from ImputePilot_api.ImputePilot_code.Labeling.AbstractLabeler import AbstractLabeler
...
class ImputeBenchLabeler(AbstractLabeler):
    ...
    LABELS_DIR = normp(os.path.join(_CURRENT_DIR, 'labels/'))
    LABELS_FILENAMES_ID = '_ibl'
    LABELS_OVERRIDE_SUFFIX = '_override'
    CONF = Utils.read_conf_file('imputebenchlabeler')
    
    _bench_path = CONF.get('BENCHMARK_PATH', '')
    if _bench_path and not os.path.isabs(_bench_path):
        # Relative path
        from django.conf import settings
        CONF['BENCHMARK_PATH'] = normp(os.path.join(str(settings.BASE_DIR), _bench_path))
    ...
    def save_labels_override(self, dataset_name, labels):
        """
        Saves override labels to CSV. These labels take priority over ImputeBench results.
        """
        labels_filename = self._get_labels_override_filename(dataset_name)
        labels.to_csv(labels_filename, index=False)

    def clear_labels_override(self, dataset_name):
        """
        Removes override labels file if it exists.
        """
        labels_filename = self._get_labels_override_filename(dataset_name)
    ...
    def load_labels(self, dataset, properties):
        ...
        # override labels if present
        override_filename = self._get_labels_override_filename(dataset.name)
        if os.path.isfile(override_filename):
            try:
                override_df = pd.read_csv(override_filename)
                if not override_df.empty:
                    # locate label column
                    label_col = None
                    for candidate in ('Label', 'label', 'bestAlgo', 'best_algo'):
                        if candidate in override_df.columns:
                            label_col = candidate
                            break
                    if label_col is None:
                        raise ValueError("Override labels file missing Label column")

                    override_df = override_df[['Cluster ID', label_col]].rename(columns={label_col: 'Label'})
                    override_df['Cluster ID'] = pd.to_numeric(override_df['Cluster ID'], errors='coerce')
                    override_df = override_df.dropna(subset=['Cluster ID'])
                    override_df['Cluster ID'] = override_df['Cluster ID'].astype(int)

                    override_df['Label'] = override_df['Label'].astype(str)
                    override_series = override_df.set_index('Cluster ID')['Label']

                    merged_labels = dict(clusters_labels_dict)
                    for cid, label in override_series.items():
                        if label is None:
                            continue
                        label_str = str(label).strip()
                        if not label_str or label_str.lower() == 'nan':
                            continue
                        merged_labels[cid] = label_str

                    clusters_labels_dict = merged_labels
                    algorithms_list = sorted({str(v) for v in merged_labels.values() if v is not None and str(v).strip()})
            except Exception as e:
                print(f"[WARN] Failed to apply override labels for {dataset.name}: {e}")
    ...
    def _compute_errors_python(self, results_folder_path):
        """Replaces error_calculation.r — computes MSE/RMSE/MAE from recovered matrices."""
        import re

        r_script_path = os.path.join(results_folder_path, 'scripts', 'precision', 'error_calculation.r')
        if not os.path.isfile(r_script_path):
            print(f"[WARN] R script not found at {r_script_path}, skipping error computation")
            return

        with open(r_script_path, 'r') as f:
            r_content = f.read()

        m = re.search(r'seq\.int\(from\s*=\s*(\d+),\s*to\s*=\s*(\d+),\s*by\s*=\s*(\d+)\)', r_content)
        if not m:
            print("[WARN] Could not parse lengths from R script")
            return
        lengths = list(range(int(m.group(1)), int(m.group(2)) + 1, int(m.group(3))))

        m = re.search(r'list_algos\s*<-\s*c\(([^)]+)\)', r_content)
        if not m:
            print("[WARN] Could not parse list_algos from R script")
            return
        list_algos = [s.strip().strip('"') for s in m.group(1).split(',')]
        ...
        for subdir, prefix, compute_fn in error_defs:
            out_dir = os.path.join(results_folder_path, 'error', subdir)
            os.makedirs(out_dir, exist_ok=True)

            for algo in list_algos:
                with open(os.path.join(out_dir, f'{prefix}{algo}.dat'), 'w') as f:
                    f.write(f'# {algo}\n')
            ...
            for length in lengths:
                ...
                if not os.path.isfile(mat_path):
                    continue
                df = pd.read_csv(mat_path, header=None, sep=r'\s+')
                ref = df.iloc[:, 0].values

                for i, algo in enumerate(list_algos):
                    diff = df.iloc[:, i + 1].values - ref
                    val = compute_fn(diff)
                    with open(os.path.join(out_dir, f'{prefix}{algo}.dat'), 'a') as f:
                        f.write(f'{length} {val}\n')

        print(f"[INFO] Error metrics computed (Python) for {os.path.basename(results_folder_path)}")
    ...
    def _run_benchmark(self, timeseries, alg_algx_cmd, scenario, errors, id, plots=False, delete_results=True):
        ...
        try:
            ...
            try:
                ...
                print(f"[WARN] Benchmark exited with code {e.returncode}, "
                  f"continuing to parse results...")

            # get results
            all_results_folder_path = normp(ImputeBenchLabeler.CONF['BENCHMARK_PATH'] + f'/Results/{scenario}/{dataset_name}/')
            
            self._compute_errors_python(all_results_folder_path)
    ...
    def _get_labels_override_filename(self, dataset_name):
        """
        Returns the filename of the override labels for the given data set's name.
        """
        return normp(
            ImputeBenchLabeler.LABELS_DIR + \
            f'/{dataset_name}{ImputeBenchLabeler.LABELS_FILENAMES_ID}{ImputeBenchLabeler.LABELS_OVERRIDE_SUFFIX}{AbstractLabeler.LABELS_APPENDIX}')
