...
from kshape.core import kshape as kshape_gpu
# from kshape.core_gpu import kshape as kshape_gpu    # use CPU
import math
...
from ImputePilot_api.ImputePilot_code.Clustering.AbstractClustering import AbstractClustering
from ImputePilot_api.ImputePilot_code.Clustering.ConFree_kClustering import cluster as cfkc_cluster
    ...
    def _clustering_reruns(self, dataset, nb_clusters, nb_reruns, accept_small_clusters=True, save_best_tocsv=False):
        ...
        while id_run < nb_reruns:
            if log_interval > 0 and time.time() - last_log >= log_interval:
                print(
                    f"[Progress] Clustering {dataset.name}: "
                    f"nb_clusters={nb_clusters}, run={id_run}/{nb_reruns}"
                )
                last_log = time.time()
            # cluster dataset
            ...
            if dataset_mean_ncc_score < ShapeBasedClustering.CONF['NCC_MIN_THRESHOLD']:
                ...
                try:
                    for nb_clusters in ds_nb_clusters_gridsearch:
                        ...
                        print(f"[Progress] Gridsearch {dataset.name}: evaluating K={nb_clusters} with {gs_nb_runs} runs")
