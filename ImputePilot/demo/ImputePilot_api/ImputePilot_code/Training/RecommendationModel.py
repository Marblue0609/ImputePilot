...
class RecommendationModel:
    ...
    def get_recommendations(self, X, use_pipeline_prod=True):
        ...
        if self.type == 'classifier':
            ...
            # Fallback 1: use final estimator's classes when pipeline.classes_ is inconsistent.
            if len(classes) != n_proba_cols and hasattr(model, 'steps') and len(model.steps) > 0:
                try:
                    final_estimator = model.steps[-1][1]
                    est_classes = list(getattr(final_estimator, 'classes_', []))
                    if len(est_classes) == n_proba_cols:
                        classes = est_classes
                except Exception:
                    pass

            # Fallback 2: if labels_set exactly matches probability width, use it.
            if len(classes) != n_proba_cols and self.labels_set is not None:
                try:
                    labels_set = list(self.labels_set)
                    if len(labels_set) == n_proba_cols:
                        classes = labels_set
                except Exception:
                    pass

            # If still inconsistent, skip this pipeline gracefully instead of raising.
            if len(classes) != n_proba_cols:
                print('[WARN] Skipping pipeline recommendation due to class/proba mismatch: '
                      f'proba_cols={n_proba_cols}, classes={len(classes)}')
                return pd.DataFrame(index=range(y_pred_proba.shape[0]))
    ...
    def _custom_predict_proba(self, _trained_pipeline):
        ...
        try:
            ...
            def _custom_predict_proba_internal(x):
                ...
                prob_pos = np.asarray(prob_pos, dtype='float64')
                denom = prob_pos.max() - prob_pos.min()
                if denom == 0:
                    prob_pos = np.full_like(prob_pos, 0.5, dtype='float64')
                else:
                    prob_pos = (prob_pos - prob_pos.min()) / denom

                # Binary decision functions often return shape (n_samples,).
                # Convert to a 2D probability matrix matching the classifier classes.
                if prob_pos.ndim == 1:
                    n_classes = len(getattr(_trained_pipeline, 'classes_', []))
                    if n_classes == 1:
                        prob_pos = prob_pos.reshape(-1, 1)
                    else:
                        prob_pos = np.vstack([1.0 - prob_pos, prob_pos]).T
                return prob_pos
